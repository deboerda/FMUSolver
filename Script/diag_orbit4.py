"""Deep-dive: why does Year alone cause failure? And what's its actual default?"""
...
# Print Year's metadata
v = var_map['Year']
print(f"Year: type={getattr(v,'type',None)!r} typeName={getattr(v,'typeName',None)!r} "
      f"causality={v.causality} variability={getattr(v,'variability',None)!r} "
      f"start={getattr(v,'start',None)!r}", flush=True)
...
# Test Year with different values
print("\n=== Year value tests ===", flush=True)
# First read the DEFAULT value from the model
v_year = var_map['Year']
print(f"Default start value for Year: {getattr(v_year, 'start', 'N/A')!r}", flush=True)

# Try Year=2024 (what we're setting)
try_init("Year=2024",   {"Year": 2024})
try_init("Year=2023",   {"Year": 2023})
try_init("Year=2100",   {"Year": 2100})
try_init("Year=2000",   {"Year": 2000})
try_init("Mon+Day",     {"Mon": 4, "Day": 10})
try_init("Mon+Day+Year",{"Mon": 4, "Day": 10, "Year": 2024})
try_init("all orbits",  {"a_MT": 6766.71777, "e_MT": 0.0011, "i_MT": 0.72653,
                          "OMEGA_MT": 2.39969, "omega_MT": 1.34409, "f_MT": 1.00447})
try_init("date+orbits", {"Year": 2024, "Mon": 4, "Day": 10, "Hour": 8, "Min": 0, "Sec": 0.0,
